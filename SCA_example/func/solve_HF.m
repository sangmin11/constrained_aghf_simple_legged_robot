...
for i =1: xpoints_new-1
    ...
    if (isKey(extra_params,'Quat_ind0'))
        ...
        if (isKey(extra_params,'w_ind0'))
            ...
            w_star = [0 0 pi/2]';
            %w_star = [0 0 0]';
            admissible_omega_ind = find(admissible_control<8);
            omega(admissible_control(admissible_omega_ind)-4) = budrift(admissible_omega_ind,i) + w_star(admissible_control(admissible_omega_ind)-4);
        end
...
%-----------------calculate cost------------------------------%
cost_int_points = xpoints;
%cost_int_points = xpoints_new;
cost_x = x;
%cost_x = xnew;
X_temp = zeros(N,cost_int_points);
dX_temp = zeros(N,cost_int_points);
...
%cost_force = zeros(1,tpoints);
%int_pd = zeros(3,tpoints);
for kk = 1:tpoints
    for j = 1:N
    [X_temp(j,:), dX_temp(j,:)]=pdeval(m,x,sol(kk,:,j),cost_x);
    end
    for i = 1:cost_int_points
        tt = t0 + (i-1)*(T/(cost_int_points-1));
        X = X_temp(:,i);
        dX = dX_temp(:,i);
        f = get_f(tt,X);
        F = get_F(tt,X);
        ...
        cost_all(kk) = cost_all(kk) + (dX-f)'*H*(dX-f)*(T/(cost_int_points-1));
        %cost_u(kk) = cost_u(kk) + udrift'*udrift*(T/(xpoints-1));
        cost_u(kk) = cost_u(kk) + 100*(udrift'*udrift)*(T/(cost_int_points-1)); % hard coded
        %cost(kk) = cost(kk) + (dX-f)'*G*(dX-f)*(T/(xpoints-1));
        cost(kk) = cost(kk) + L*(T/(cost_int_points-1));
        %cost_force(kk) = cost_force(kk) + (dX(1:3)-[1 0 0]')'*[0 0 -500]'*(T/(xpoints-1));
        %int_pd(:,kk) = int_pd(:,kk) + (dX(1:3)-[1 0 0]')*(T/(xpoints-1));
        
    end
end

% get more accurate cost for final solution
final_cost = 0; %total cost(includes barrier and full control)
final_cost_all = 0;% cost using full control only
final_cost_u = 0; % cost using only admissible control
for i = 1:xpoints_new
    tt = t0 + (i-1)*(T/(xpoints_new-1));
    X = p(:,i);
    dX = dp(:,i);
    ...
    ufull = F^(-1) * ( dX - f );
    udrift= ufull((end-M+1));
    final_cost_all = final_cost_all + (dX-f)'*H*(dX-f)*(T/(xpoints_new-1));
    %cost_u(kk) = cost_u(kk) + udrift'*udrift*(T/(xpoints-1));
    final_cost_u = final_cost_u + 100*(udrift'*udrift)*(T/(xpoints_new-1)); % hard coded
    %cost(kk) = cost(kk) + (dX-f)'*G*(dX-f)*(T/(xpoints-1));
    final_cost = final_cost + L*(T/(xpoints_new-1));
    %cost_force(kk) = cost_force(kk) + (dX(1:3)-[1 0 0]')'*[0 0 -500]'*(T/(xpoints-1));
    %int_pd(:,kk) = int_pd(:,kk) + (dX(1:3)-[1 0 0]')*(T/(xpoints-1));
end
display(['final total cost: ' num2str(final_cost)]);
display(['final cost due to all control: ' num2str(final_cost_all)]);
display(['final cost due to admissible control: ' num2str(final_cost_u)]);
